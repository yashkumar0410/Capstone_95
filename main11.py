...
key_points = [
    ...
    # left attack line
    (int((6/actual_width)*court_w), court_h),
    (int((6/actual_width)*court_w),0),
...
while cap.isOpened():
    ...
    for r in court_results:
        ...
        for box in boxes:
            ...
            top_left = (x1+205, y1)
            top_right = (x2, y1)
            bottom_left = (x1, y2-105)
            bottom_right = (x2, y2)
            # -------------------------
            # Draw corners
            # -------------------------
            cv2.circle(annotated_frame, top_left, 5, (0, 0, 255), -1)
            cv2.putText(annotated_frame, "Top Left", (top_left[0] + 5, top_left[1] - 5),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

# Top Right
            cv2.circle(annotated_frame, top_right, 5, (255, 0, 0), -1)
            cv2.putText(annotated_frame, "Top Right", (top_right[0] + 5, top_right[1] - 5),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

# Bottom Left
            cv2.circle(annotated_frame, bottom_left, 5, (0, 255, 0), -1)
            cv2.putText(annotated_frame, "Bottom Left", (bottom_left[0] + 5, bottom_left[1] - 5),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

# Bottom Right
            cv2.circle(annotated_frame, bottom_right, 5, (0, 255, 255), -1)
            cv2.putText(annotated_frame, "Bottom Right", (bottom_right[0] + 5, bottom_right[1] - 5),
